...
we must known these mysql syntaxes for this project

1. enter password
2.create database databasename;*/(database name for this project           is "vogue")
3.use vogue;
4.create table table name(write it here what we are need for rows);
    we need to create four tables like( bill_items, mens_wear, women_wear, sign_up)

5.lock tables table name write;
6.insert into tablename values(what we need);
7.unlock tables;

extra used syntax for knowing bout it

1.drop table tablename;(for deleting table)
2.decribe tablename;(for known what is present in table)
3.select *from tablename;
4.delete from tablename where uniquename(like primary keys etc)=that neme Or id
